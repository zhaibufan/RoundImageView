package com.example.zhai.roundimageview;
...
/**
 * Created by Mastra on 2017/11/13.
 */
...
public class RoundImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    private void setShader() {
        ...
        float scale = 1.0f;
        if (type == TYPE_ROUND) {
            // 取最大值，使其放大或缩小后能覆盖view
            scale = Math.max(bitmap.getWidth() * 1.0f / getWidth(), bitmap.getHeight() * 1.0f / getHeight());
        } else if (type == TYPE_CIRCLE) {
            // 取图片和view宽高中的最小值，进行缩放
            int bitmapSize = Math.min(bitmap.getWidth(), bitmap.getHeight());
            int viewSize = Math.min(getHeight(), getWidth());
            int bitmapWidth = Math.min(bitmapSize, viewSize);
            scale = mWidth * 1.0f / bitmapWidth;
        }
        // matrix的作用是为了避免view的宽高大于图片的宽高而导致图片被拉伸，
        // 因为我们会为BitmapShader设置一个matrix，可以去适当的放大或者缩小图片
